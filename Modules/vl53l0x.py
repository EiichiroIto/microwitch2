from micropython import const
import ustruct
import utime

class TimeoutError(RuntimeError):
    pass

class VL53L0X:
    def __init__(self, i2c, addr=0x29):
        self.i2c = i2c
        self.addr = addr
        self.init()
        self._started = False

    def _regs(self, register, values=None, struct='B'):
        if values is None:
            size = ustruct.calcsize(struct)
            data = self.i2c.readfrom_mem(self.addr, register, size)
            values = ustruct.unpack(struct, data)
            return values
        data = ustruct.pack(struct, *values)
        self.i2c.writeto_mem(self.addr, register, data)

    def _reg(self, register, value=None, struct='B'):
        if value is None:
            return self._regs(register, struct=struct)[0]
        self._regs(register, (value,), struct=struct)

    def _flag(self, register=0x00, bit=0, value=None):
        data = self._reg(register)
        mask = 1 << bit
        if value is None:
            return bool(data & mask)
        elif value:
            data |= mask
        else:
            data &= ~mask
        self._reg(register, data)

    def _config(self, *config):
        for register, value in config:
            self._reg(register, value)

    def init(self, power2v8=True):
        self._flag(const(0x89), 0, power2v8)
        self._config( (0x88, 0x00), (0x80, 0x01), (0xff, 0x01), (0x00, 0x00), )
        self._stop_variable = self._reg(0x91)
        self._config( (0x00, 0x01), (0xff, 0x00), (0x80, 0x00), )
        self._flag(const(0x60), 1, True)
        self._flag(const(0x60), 4, True)
        self._reg(const(0x44), int(0.25 * (1 << 7)), struct='>H')
        self._reg(const(0x01), 0xff)
        spad_count, is_aperture = self._spad_info()
        spad_map = bytearray(self._regs(const(0xb0), struct='6B'))
        self._config( (0xff, 0x01), (const(0x4f), 0x00), (const(0x4e), 0x2c), (0xff, 0x00), (const(0xb6), 0xb4), )
        spads_enabled = 0
        for i in range(48):
            if i < 12 and is_aperture or spads_enabled >= spad_count:
                spad_map[i // 8] &= ~(1 << (i >> 2))
            elif spad_map[i // 8] & (1 << (i >> 2)):
                spads_enabled += 1
        self._regs(const(0xb0), spad_map, struct='6B')
        self._config( (0xff, 0x01), (0x00, 0x00), (0xff, 0x00), (0x09, 0x00), (0x10, 0x00), (0x11, 0x00), (0x24, 0x01), (0x25, 0xFF),
            (0x75, 0x00), (0xFF, 0x01), (0x4E, 0x2C), (0x48, 0x00), (0x30, 0x20), (0xFF, 0x00), (0x30, 0x09), (0x54, 0x00),
            (0x31, 0x04), (0x32, 0x03), (0x40, 0x83), (0x46, 0x25), (0x60, 0x00), (0x27, 0x00), (0x50, 0x06), (0x51, 0x00),
            (0x52, 0x96), (0x56, 0x08), (0x57, 0x30), (0x61, 0x00), (0x62, 0x00), (0x64, 0x00), (0x65, 0x00), (0x66, 0xA0),
            (0xFF, 0x01), (0x22, 0x32), (0x47, 0x14), (0x49, 0xFF), (0x4A, 0x00), (0xFF, 0x00), (0x7A, 0x0A), (0x7B, 0x00),
            (0x78, 0x21), (0xFF, 0x01), (0x23, 0x34), (0x42, 0x00), (0x44, 0xFF), (0x45, 0x26), (0x46, 0x05), (0x40, 0x40),
            (0x0E, 0x06), (0x20, 0x1A), (0x43, 0x40), (0xFF, 0x00), (0x34, 0x03), (0x35, 0x44), (0xFF, 0x01), (0x31, 0x04),
            (0x4B, 0x09), (0x4C, 0x05), (0x4D, 0x04), (0xFF, 0x00), (0x44, 0x00), (0x45, 0x20), (0x47, 0x08), (0x48, 0x28),
            (0x67, 0x00), (0x70, 0x04), (0x71, 0x01), (0x72, 0xFE), (0x76, 0x00), (0x77, 0x00), (0xFF, 0x01), (0x0D, 0x01),
            (0xFF, 0x00), (0x80, 0x01), (0x01, 0xF8), (0xFF, 0x01), (0x8E, 0x01), (0x00, 0x01), (0xFF, 0x00), (0x80, 0x00),
        )
        self._reg(const(0x0a), 0x04)
        self._flag(const(0x84), 4, False)
        self._reg(const(0x0b), 0x01)
        self._reg(const(0x01), 0x01)
        self._calibrate(0x40)
        self._reg(const(0x01), 0x02)
        self._calibrate(0x00)
        self._reg(const(0x01), 0xe8)

    def _spad_info(self):
        self._config( (0x80, 0x01), (0xff, 0x01), (0x00, 0x00), (0xff, 0x06), )
        self._flag(0x83, 3, True)
        self._config( (0xff, 0x07), (0x81, 0x01), (0x80, 0x01), (0x94, 0x6b), (0x83, 0x00), )
        for timeout in range(1000):
            if self._reg(0x83):
                break
            utime.sleep_ms(1)
        else:
            raise TimeoutError()
        self._config( (0x83, 0x01), )
        value = self._reg(0x92)
        self._config( (0x81, 0x00), (0xff, 0x06), )
        self._flag(0x83, 3, False)
        self._config( (0xff, 0x01), (0x00, 0x01), (0xff, 0x00), (0x80, 0x00), )
        count = value & 0x7f
        is_aperture = bool(value & 0b10000000)
        return count, is_aperture

    def _calibrate(self, vhv_init_byte):
        self._reg(const(0x00), 0x01 | vhv_init_byte)
        for timeout in range(1000):
            if self._reg(const(0x13)) & 0x07:
                break
            utime.sleep_ms(1)
        else:
            raise TimeoutError()
        self._reg(const(0x0b), 0x01)
        self._reg(const(0x00), 0x00)

    def start(self, period=0):
        self._config( (0x80, 0x01), (0xFF, 0x01), (0x00, 0x00), (0x91, self._stop_variable), (0x00, 0x01), (0xFF, 0x00), (0x80, 0x00), )
        if period:
            oscilator = self._reg(const(0xf8), struct='>H')
            if oscilator:
                period *= oscilator
            self._reg(const(0x04), period, struct='>H')
            self._reg(const(0x00), 0x04)
        else:
            self._reg(const(0x00), 0x02)
        self._started = True

    def stop(self):
        self._reg(const(0x00), 0x01)
        self._config( (0xFF, 0x01), (0x00, 0x00), (0x91, self._stop_variable), (0x00, 0x01), (0xFF, 0x00), )
        self._started = False

    def read(self):
        if not self._started:
            self._config( (0x80, 0x01), (0xFF, 0x01), (0x00, 0x00), (0x91, self._stop_variable), (0x00, 0x01), (0xFF, 0x00), (0x80, 0x00), (const(0x00), 0x01), )
            for timeout in range(1000):
                if not self._reg(const(0x00)) & 0x01:
                    break
                utime.sleep_ms(1)
            else:
                raise TimeoutError()
        for timeout in range(1000):
            if self._reg(const(0x13)) & 0x07:
                break
            utime.sleep_ms(1)
        else:
            raise TimeoutError()
        value = self._reg(const(0x14) + 10, struct='>H')
        self._reg(const(0x0b), 0x01)
        return value
