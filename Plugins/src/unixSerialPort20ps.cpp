...
extern "C" {
  ...
  int WriteHexCode(char *filename, char *buf, int size);
  void ShowErrno();
};
...
int isSerialPortDev(char *s) {
  return isPrefix((char*) "ttyusb", s) || isPrefix((char*) "ttyacm", s);
}
...
int
WriteHexCode(char *filename, char *buf, int size)
{
  int fd, ret;

  printf("WriteHexCode(%s,%20.20s,%d)\n", filename, buf, size);
